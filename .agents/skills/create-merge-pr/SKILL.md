...
```
uv run --all-extras ruff check redis_fifo_lock/ tests/ && uv run --all-extras ruff format --check redis_fifo_lock/ tests/
uv run --all-extras pytest
```

Note: tests need a reachable Redis (`REDIS_URL`, default
`redis://localhost:6379/15`). CI provides one as a service container.
